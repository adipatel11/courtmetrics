       ...
       "Statement": [
         {
           ...
           "Resource": "arn:aws:dynamodb:REGION:ACCOUNT_ID:table/tsv-users"
         }
       ]
     }
     ```
     Replace `REGION`, `ACCOUNT_ID`, and `tsv-users` with your actual values, save, then attach this policy to the new user.
...
2. Run `npm run dev`.
3. In the browser, go to `http://localhost:3000`, create an account, then check DynamoDB → Tables → `tsv-users` → `Explore table items` to confirm a new record.
